...
function the_champ_connect(){
	...
	if(isset($_GET['code']) && isset($_GET['state'])){
		...
		if($line_redirect_url != home_url().'/SuperSocializerAuth/Line'){
			...
			if('super_socializer_google' == $_GET['state']){
			    ...
			    if(is_object($googleClient)){
				    ...
				    if($accessTokenStr){
				        ...
				        if(is_object($userData) && isset($userData->id)){
							...
							$googleRedirectUrl = isset($_SESSION['super_socializer_google_redirect']) ? esc_url(trim($_SESSION['super_socializer_google_redirect'])) : esc_url(home_url());
